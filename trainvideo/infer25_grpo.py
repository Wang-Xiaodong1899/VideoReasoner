...
# /mnt/bn/multimodal-datasets-hl/wangxd/Open-R1-Video-Mix/assets/fFjv93ACGo8.mp4
...
def extract_before_second_event_start(text):
    marker = "<|event_start|>"
    first_index = text.find(marker)
    if first_index == -1:
        return None  # 没有出现
    second_index = text.find(marker, first_index + len(marker))
    if second_index == -1:
        return None  # 只有一次出现
    return text[:second_index]  # 返回第二次出现前的内容
...
if match:
    ...
    prefix = extract_before_second_event_start(truncated_text[0]) + "<|event_start|>" + f"{timeline}" + "<|event_end|>"
    truncated_text[0] = prefix  + ". Focusing on this timeframe <|video_zoomin|><|segment_pad|>"
    print(truncated_text)
else:
    print("未找到符合标签要求的内容")
