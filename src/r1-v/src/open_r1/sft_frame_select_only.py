...
from qwen_vl_utils import process_vision_info, process_vision_keyframes_info
...
def prepare_dataset(example: Dict[str, Any]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    # Please show your reasoning and answer in the following tags: <think>The reasoning goes here</think> <answer>The answer goes here</answer>. <video>Question:
    ...
    #         "max_pixels": 150*28*28,
    #         "fps": 2.0,
    #         "max_frames": 16,
    #     })
    if 'keyframes' in example and example['keyframes']:
        keyframes = example['keyframes'] # int list
    ...
    return {"messages": messages, "keyframes": keyframes}
...
def collate_fn(examples: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
    ...
    for i, example in enumerate(examples):
        try:
            ...
            # keyframes_inputs should be a list of pil images

            image_inputs, video_inputs, keyframes_inputs, video_kwargs = process_vision_keyframes_info(example["messages"], example["keyframes"], return_video_kwargs=True)
            
            # import pdb; pdb.set_trace()
            # segment_inputs: [torch.Size([30, 3, 252, 448])]
    ...
    start_token_id = processor.tokenizer.convert_tokens_to_ids("<|keyframe_start|>")
    end_token_id = processor.tokenizer.convert_tokens_to_ids("<|keyframe_end|>")

    for i in range(labels.size(0)):
        label_seq = labels[i]
        # 找到所有 start 和 end 的索引
        start_indices = (label_seq == start_token_id).nonzero(as_tuple=True)[0]
        end_indices = (label_seq == end_token_id).nonzero(as_tuple=True)[0]

        # 确保配对存在且数量匹配
        for start_idx, end_idx in zip(start_indices, end_indices):
            if start_idx <= end_idx:
                label_seq[start_idx:end_idx + 1] = -100
    ...
    visual_tokens = [processor.tokenizer.convert_tokens_to_ids(token) for token in ["<|keyframe_start|>", "<|keyframe_end|>", "<|keyframes_pad|>", "<|segment_pad|>", "<|vision_start|>", "<|vision_end|>", "<|vision_pad|>", "<|image_pad|>", "<|video_pad|>"]]
...
if __name__ == "__main__":
    ...
    model.config.keyframe_token_id = processor.tokenizer.convert_tokens_to_ids("<|keyframes_pad|>") # <|segment_pad|>
    
    print(f"model.config.keyframe_token_id: {model.config.keyframe_token_id}")

    x_token_id = processor.tokenizer.convert_tokens_to_ids("<|event_start|>") # <|event_start|>
    print(f"<|event_start|>: {x_token_id}")
    x_token_id = processor.tokenizer.convert_tokens_to_ids("<|event_end|>") # <|event_end|>
    print(f"<|event_end|>: {x_token_id}")
    x_token_id = processor.tokenizer.convert_tokens_to_ids("<|video_zoomin|>") # <|video_zoomin|>
    print(f"<|video_zoomin|>: {x_token_id}")

    x_token_id = processor.tokenizer.convert_tokens_to_ids("<|keyframe_selection_tool|>") # <|video_zoomin|>
    print(f"<|keyframe_selection_tool|>: {x_token_id}")
    x_token_id = processor.tokenizer.convert_tokens_to_ids("<|keyframe_start|>") # <|video_zoomin|>
    print(f"<|keyframe_start|>: {x_token_id}")
    x_token_id = processor.tokenizer.convert_tokens_to_ids("<|keyframe_end|>") # <|video_zoomin|>
    print(f"<|keyframe_end|>: {x_token_id}")
    x_token_id = processor.tokenizer.convert_tokens_to_ids("<|keyframes_embed|>") # <|video_zoomin|>
    print(f"<|keyframes_embed|>: {x_token_id}")
    x_token_id = processor.tokenizer.convert_tokens_to_ids("<|keyframes_pad|>") # <|video_zoomin|>
    print(f"<|keyframes_pad|>: {x_token_id}")
