...
def prepare_dataset(example: Dict[str, Any]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    system_message = "You are a very smart multimodal assistant that can understand images and videos. You should first thinks about the reasoning process in the mind and then provides the user with the answer. "
    ...
    question = example['messages'][1]['content'].split("please answer the following question:")[-1].strip()

    r1_output = example['messages'][2]['content']
    ...
    messages = [
        ...
        },
        {
            ...
            "content": [
                {
                    "type": 'image', # hard code
                    "image": os.path.join("/mnt/bn/multimodal-datasets-hl/wangxd/data/LLaVA-R1-100k/images", example['image'])
                    # "max_pixels": 360*420,
                    # "fps": 1.0
                },
                {
        ...
        {
            "role": "assistant",
            "content": [{"type": "text", "text": r1_output}]
        }
    ]
...
def collate_fn(examples: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
    ...
    if image_inputs is None and video_inputs is None:
        ...
        dummy_input["labels"][dummy_input["labels"] == processor.tokenizer.pad_token_id] = -100
        return dummy_input
