...
def prepare_dataset(example: Dict[str, Any]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    # "{Question}\n"
    # "Your final response should follow the format: <answer> answer </answer>."
    # )
    # VIDEO_QUESTION_TEMPLATE = (
    ...
    #     "3. **Direct answer**: If the question can be answered directly without additional processing, provide the answer clearly.\n"
    #     "Question: {Question}\n"
    # )
    VIDEO_QUESTION_TEMPLATE = (
        "Given a video, please analyze the content carefully and provide your response in one of the following formats:\n"
    ...
    if 'segment_path' in example and example['segment_path']:
        ...
        #     "max_frames": 16,
        # })
        # extra_data contain duration, times, points
        extra_data = {
    ...
    return {"messages": messages, "extra_data": extra_data}
...
def collate_fn(examples: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
    ...
    for i, example in enumerate(examples):
        try:
            ...
            image_inputs, video_inputs, segment_inputs, _, video_kwargs = process_vision_segment_info(example["messages"], return_video_kwargs=True)
            
            extra_data = example['extra_data']
            if extra_data:
                # video_kwargs['duration'] = extra_data['duration']
                video_kwargs['times'] = extra_data['times']
