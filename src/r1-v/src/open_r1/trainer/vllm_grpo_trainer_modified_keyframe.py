...
class Qwen2VLGRPOVLLMTrainerModified(Trainer):
    def __init__(
        ...
        # NOTE close grad before use deepspeed
        for name, param in model.named_parameters():
            # XXX visual.blocks, visual.merger
            # if "visual" in name:
        ...
        self.use_std = script_args.use_std
        self.use_length_norm = script_args.use_length_norm
        ...
        print(f"len_control: {self.len_control}")
        self.beta = args.beta
        ...
        # check grad for self.model.visual
        # blocks = self.model.visual.blocks
        # # check each block's requires_grad
        # for idx, block in enumerate(blocks):
        #     requires_grad = any(p.requires_grad for p in block.parameters())
        #     print(f"Block visual.blocks[{idx}] requires_grad: {requires_grad}")

        # for block in self.model.visual.blocks:
        #     for param in block.parameters():
        #         param.requires_grad = True

        # print('-------------set visual.blocks requires_grad to True----------------')
        ...
        # print(f"Total parameters in 'visual.blocks': {total_params}")
        # print(f"Trainable parameters in 'visual.blocks': {trainable_params}")
    ...
    def compute_loss(
        ...
        input_copy = copy.deepcopy(inputs[0]['messages'])
        
        input_copy = self.remove_none_from_data(input_copy)

        # import pdb; pdb.set_trace()

        # NOTE cancel the add_generation_prompt
        prompts_text = self.processor.apply_chat_template(input_copy, tokenize=False, add_generation_prompt=False)
        # print(f"{input_copy} --> {prompts_text}")

        prompts_text = [prompts_text]

        prompts_text[0] = prompts_text[0] + inputs[0]['prefix']

        # import pdb; pdb.set_trace()
        
        # if no system prompt, using input_copy[0]
        ...
        image_inputs, video_inputs, keyframes_inputs, video_kwargs = process_vision_keyframes_info(input_copy, inputs[0]['keyframe_indices_list'], return_video_kwargs=True)
        if len(inputs[0]['keyframe_indices_list']) == 0:
            keyframes_inputs = None
        ...
        # TODO fix multiple mm_data
        # mm_data = [[data_type, image_inputs if image_inputs else video_inputs]]
        mm_data = [['image', keyframes_inputs, 'video', video_inputs]] if keyframes_inputs is not None else [['video', video_inputs]]
        ...
        if self.args.use_vllm:
            ...
            for prompt, mm_item in zip(all_prompts_text, all_mm_data):
                ...
                if len(mm_item) > 2:
                    all_multimodal_inputs.append({"prompt": prompt, "multi_modal_data": {mm_item[0]: mm_item[1], mm_item[2]: mm_item[3]}})
                else:
                    all_multimodal_inputs.append({"prompt": prompt, "multi_modal_data": {mm_item[0]: mm_item[1]}})
        ...
        data_type = 'image'
        # should repeat both
        if data_type == 'image' and keyframes_inputs is not None:
        ...
        # data repeat for get token logps
        ...
        completions = self.processing_class.batch_decode(
            completion_ids, skip_special_tokens=False # Need False
        )
        if is_conversational(inputs[0]):
            completions = [
        ...
        if self.len_control:
            ...
            if len(selected_indices) > 1:     
                for idx in selected_indices:
                    if 320 <= lenth_list[idx] <= 512: # NOTE implicit length encourage
                        # if 450 <= lenth_list[idx] <= 700: # NOTE large
                        rewards[idx] += 0.2
        ...
        # using std, affect the advantage
        if self.use_std:
        ...
        # NOTE reward_per_func[0] --> accuracy
        
        # gathered_rewards = self.accelerator.gather_for_metrics(rewards)

        # NOTE only plot accuracy reward for all_wrong and all_correct
        gathered_rewards = self.accelerator.gather_for_metrics(rewards_per_func)[:, 0]
    ...
    #     loss = super().training_step(model, inputs, num_items_in_batch)  # 内部已经做了 forward/backward
    #     # 这时 loss.backward() 已经执行完，梯度尚未被 zero_grad()
    #     for n, p in model.named_parameters():
    #         print(f"{n}: has_grad={p.grad is not None}", flush=True)
