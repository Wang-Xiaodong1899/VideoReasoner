...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if processing_class is None:
            if "Qwen2-VL" in model_id or "Qwen2VL" in model_id or "Qwen2.5-VL" in model_id or "Aria" in model_id or True:
        ...
        self.generation_config = GenerationConfig(
            ...
            do_sample=True,
            top_p=0.95,  
            temperature=script_args.sample_temperature, # HACK
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        #     input_copy[0]['content'][0]['image'] = "xxx"
        # elif inputs[0]['data_type'] == 'video':
        #     input_copy[0]['content'][0]['video'] = "xxx"
        ...
        prompts_text = self.processing_class.apply_chat_template(input_copy, tokenize=False, add_generation_prompt=False)
        
        prompts_text = [prompts_text]

        generate_prefix = inputs[0]['prefix']

        prompts_text[0] = prompts_text[0] + "<|im_start|>assistant\n" + generate_prefix # let model auto output xxx

        times = inputs[0]['times']
        # points = inputs[0]['points']

        if len(times) > 0:
            times[-1] = times[-1] if times[-1] < inputs[0]['duration'] else inputs[0]['duration']
            print('times:', inputs[0]['times'], ' duration:', inputs[0]['duration'], 'path:', inputs[0]['path'])
        ...
        try:
            ...
            # _, segment_inputs = process_vision_given_multi_durations(input_copy, key_durations=times, return_video_kwargs=False, pad_frame=False, max_frames=16)
        ...
        if inputs[0]['data_type'] == 'video':
            ...
            # if 'pixel_values_segments' in prompt_inputs:
            #     prompt_inputs["pixel_values_segments"] = prompt_inputs["pixel_values_segments"].repeat(len(prompt_completion_ids), 1)
            #     prompt_inputs["seg_grid_thw"] = prompt_inputs["seg_grid_thw"].repeat(len(prompt_completion_ids), 1)
        ...
        completions = self.processing_class.batch_decode(completion_ids, skip_special_tokens=False) # Need False
        if is_conversational(inputs[0]):
