...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # _, segment_inputs = process_vision_given_multi_durations(input_copy, key_durations=times, return_video_kwargs=False, pad_frame=False, max_frames=32)
        _, _, keyframes_inputs, video_kwargs = process_vision_keyframes_info(input_copy, keyframes, return_video_kwargs=True)
        ...
        if inputs[0]['data_type'] == 'video':
            ...
            # for keyframes (as image) embedding
            if prompt_inputs["pixel_values"] is not None:
                prompt_inputs["pixel_values"] = prompt_inputs["pixel_values"].repeat(len(prompt_completion_ids), 1)
                prompt_inputs["image_grid_thw"] = prompt_inputs["image_grid_thw"].repeat(len(prompt_completion_ids), 1)
