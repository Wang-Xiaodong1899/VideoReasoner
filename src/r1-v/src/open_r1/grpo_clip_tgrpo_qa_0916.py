    ...
    for content, sol in zip(contents, solution):
        if question_type == 'qa':
            try:
                ...
                print(f"output: {content}, gt_ans: {gt_ans}")
                output_ans = content.strip()
                gt_ans = gt_ans.strip()
                if output_ans == "":
                    output_ans = " "
                if question_type == "multiple choice" or question_type == 'qa':
                    reward = 1.0 if output_ans[0] == gt_ans.strip()[0] else 0.0
...
def format_reward(completions, **kwargs):
    ...
    # qa: the answer is, reward 0 is ok
