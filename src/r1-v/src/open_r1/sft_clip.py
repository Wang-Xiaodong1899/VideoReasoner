...
def prepare_dataset(example: Dict[str, Any]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    VIDEO_QUESTION_TEMPLATE = (
        ...
        "If the content of the question in the video is unclear or difficult to see, first locate the key event by generating the start-frame and end-frame in the following format: <|event_start|> [start-frame, end-frame] <|event_end|>.\n"
    ...
    return {"messages": messages, "duration": example['duration'], "times": example['times']}
...
def collate_fn(examples: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
    ...
    for i, example in enumerate(examples):
        try:
            ...
            # given start(s) and end(s), transform to start(frame) and end(frame)
            start = example['times'][0]
            end = example['times'][1]
            ...
            user_content[1]["text"] = user_content[1]["text"].replace("Give you a video.", f"Given you a video with {nframes} frames.")
            ...
            example["messages"][2]["content"][0]["text"] = prefix + f"<|event_start|>[{start_frame}, {end_frame}]<|event_end|>" + tail
