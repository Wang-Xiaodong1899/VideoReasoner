...
def accuracy_reward(completions, solution, **kwargs):
    ...
    def extract_box_answer(text):
        pattern = r"\s*<answer>.*?boxed(.*?)</answer>" # only extract {xxx}
        match = re.search(pattern, text, re.DOTALL)
        if match:
    ...
    for content, sol in zip(contents, solution):
        ...
        try:
            output_ans = extract_box_answer(content)
            gt_ans = extract_answer(sol)
            # NOTE remove {}
            # output_ans = output_ans.replace('{', '').replace('}', '')
            # gt_ans = gt_ans.replace('{', '').replace('}', '')
...
def main(script_args, training_args, model_args):
    ...
    VIDEO_QUESTION_TEMPLATE = (
        "Given the video to you. Please answer the following question:\n"
        "{Question}\n"
        "The reasoning process and answer are enclosed within <think> </think> and <answer> </answer> tags, respectively, i.e., <think> reasoning process here </think> <answer> answer here </answer>. "
    )
