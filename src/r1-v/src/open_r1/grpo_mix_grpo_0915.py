...
from trainer import Qwen2VLGRPOTrainerClipTGRPO
...
class GRPOScriptArguments(ScriptArguments):
    ...
    reward_funcs: list[str] = field(
        default_factory=lambda: ["accuracy", "iou"],
...
def iou_reward(completions, **kwargs):
    def extract_pred_time(text):
        pattern = r".*?<\|event_start\|>(.*?)<\|event_end\|>.*?"
    ...
    if question_type == "grounding":
        ...
        # set time<0 --> 0
        # set time>0 --> 1
        for idx, time in enumerate(gt_times):
            if time < 0:
                gt_times[idx] = 0.0
            elif time > 1:
                gt_times[idx] = 1.0
...
def diverse_reward(completions, **kwargs):
    def diverse_score(hypothesis):
        import requests
        url = "http://localhost:8003/bertdiversescore"
        data = {
            "hypothesis": hypothesis,
        }
        response = requests.post(url, json=data)
        return response.json()["mean_distance"]
    ...
    current_time = datetime.now().strftime("%d-%H-%M-%S-%f")
    rewards = []
    beta_keyitem_reward = 0.1 # NOTE hard code 0.1
    for content in contents:
        if question_type == "keyitem":
            reward = 0.0
            score = 1 - diverse_score(content)
            reward += score * beta_keyitem_reward
        else:
            reward = 0.5 * beta_keyitem_reward # default 0.5
        rewards.append(reward)
        ...
        if os.getenv("DEBUG_MODE") == "true":
            ...
            # local_rank = int(os.getenv("LOCAL_RANK", 0))
            with open(log_path, "a", encoding="utf-8") as f:
                f.write(f"------------- {current_time} Diverse reward: {reward} -------------\n")
                f.write(f"Content: {content}\n")
                f.write(f"local_path: {local_path}\n")
    return rewards
    ...
    for content, sol in zip(contents, solution):
        if question_type == 'qa':
            try:
                ...
                # output_ans = output_ans.strip()[0]
                # gt_ans = gt_ans.strip()[0]
                print(f"output_ans: {output_ans}, gt_ans: {gt_ans}")
                if question_type == "multiple choice" or question_type == 'qa':
...
def format_reward(completions, **kwargs):
    ...
    # print(f"question_type len: {len(kwargs['problem_type'])}")

    print(f"question_type: {kwargs['problem_type']}")
...
def main(script_args, training_args, model_args):
                    ...
                    "content": [
                        {
                            "type": "video",
                            "video": example['path'],
                            "max_pixels": 80*28*28, # 128*28*28, #150*28*28,
                            "max_frames": 64 #64 #32
                        },
                        {
                            "type": "text",
    ...
    if training_args.use_vllm:
        raise ValueError("use_vllm must be False")
    trainer_cls = Qwen2VLGRPOTrainerClipTGRPO
