...
from trainer import Qwen2VLGRPOTrainerEGRPO
...
def accuracy_reward(completions, solution, **kwargs):
    ...
    def compute_f1_score(reference_list, hypothesis_list):
        list1 = hypothesis_list
        if isinstance(list1, str):
            list1 = [item.strip().strip() for item in hypothesis_list.split(',')]
        list2 = reference_list
...
"""
    ...
    for content, sol in zip(contents, solution):
        ...
        try:
            ...
            reward = compute_f1_score(key_items, content)
        except Exception as e:
            print(f"Error in reward_fn for question_type '{question_type}': {e}")
            reward = 0.0
...
def main(script_args, training_args, model_args):
    ...
    trainer_cls = Qwen2VLGRPOTrainerEGRPO
