...
def main(start=0, end=400):
    with open("/mnt/bn/ws-candy-hl-62827-yz89lqpbo2/data/Video-R1-data/Video-R1-260k-filter-video.json", "r") as f:
        data = json.load(f)
    video_convs_dict = {}
    id_count = 0
    for item in tqdm(data):
        ...
        if video not in video_convs_dict:
            ...
            video_convs_dict[video][f"{problem_id}"] = query
            id_count += 1
    count = 0
    with open(f"VideoR1-video-keyframes_idx_max_64_0627_all_id_{start}_{end}.jsonl", "w") as f:
